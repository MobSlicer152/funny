...
def main(argc: int, argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        prog="bmp2c", description="convert indexed BMP images into C sources"
    )
    ...
    symbol = (
        args.symbol
        if args.symbol != None
        else clean_name(os.path.splitext(os.path.basename(image))[0]).upper()
    )
    output = (
        args.output
        if args.output != None
        else f"{os.path.join(os.path.dirname(image), symbol.lower())}.h"
    )
